...
export default function ResultView({ project, originalUrl, onStartOver }) {
  ...
  const resultTexts = t("result", { ns: "transform", returnObjects: true }) || {};
    ...
    <div className="space-y-8">
      ...
      <Card className="p-6 bg-green-50 border-green-200">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold text-green-900">{resultTexts.title}</h3>
            <p className="text-sm text-green-700">{resultTexts.subtitle}</p>
          </div>
        </div>
      </Card>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        >
          ...
          {resultTexts.download || t("buttons.download", { ns: "common" })}
        </Button>
        <Button
          onClick={onStartOver}
          size="lg"
